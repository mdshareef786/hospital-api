...
def auth_headers(client):
    response = client.post("/auth/login", json={"username": "testadmin", "password": "test123"})
...
def test_login_wrong_password(client):
    response = client.post("/auth/login", json={"username": "testadmin", "password": "wrong"})
...
def test_update_doctor(client, auth_headers):
    ...
    response = client.put(f"/doctors/{doctor_id}", json={"specialization": "Oncology"}, headers=auth_headers)
    assert response.status_code == 200
    assert response.json()["specialization"] == "Oncology"
...
def test_toggle_doctor_status(client, auth_headers):
    ...
    response = client.patch(f"/doctors/{doctor_id}/toggle-status", headers=auth_headers)
    assert response.status_code == 200
...
def test_search_patient(client, auth_headers):
    client.post("/patients", json={"name": "Alice Wonder", "age": 28, "phone": "1111111111"}, headers=auth_headers)
...
def test_cancel_appointment(client, auth_headers):
    ...
    response = client.patch(f"/appointments/{appt['id']}/cancel", headers=auth_headers)
    assert response.status_code == 200
