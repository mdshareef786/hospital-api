...
function AppointmentModal({ onClose, onSave }) {
  ...
  useEffect(() => {
    getDoctors({ page: 1, page_size: 100, is_active: true })
      .then(r => setDoctors(r.data?.data?.items || []));
    getPatients({ page: 1, page_size: 100 })
      .then(r => setPatients(r.data?.data?.items || []));
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast(err.response?.data?.message || 'Failed to book', 'error');
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            ...
            <div className="form-group">
              ...
              <input className="form-input" type="datetime-local" value={form.appointment_date}
                onChange={e => setForm({ ...form, appointment_date: e.target.value })} required />
            </div>
            <div className="form-group">
              <label className="form-label">Notes</label>
              <textarea className="form-textarea" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="Symptoms or notes..." />
            </div>
          </div>
...
export default function AppointmentsPage() {
  ...
  const handleCancel = async (id) => {
    ...
    try { await cancelAppointment(id); toast('Cancelled', 'success'); fetchAppointments(); }
    ...
    <div className="page-body">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted">{total} total appointment{total !== 1 ? 's' : ''}</p>
        </div>
            ...
            <table>
              ...
              <tbody>
                {items.map(a => (
                  <tr key={a.id}>
                    <td>#{a.id}</td>
                    <td><strong>{a.doctor?.name || `#${a.doctor_id}`}</strong><br />
                      <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>{a.doctor?.specialization}</span></td>
                    <td>{a.patient?.name || `#${a.patient_id}`}<br />
                      <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>Age: {a.patient?.age}</span></td>
      ...
      {modal === 'create' && <AppointmentModal onClose={() => setModal(null)} onSave={() => { setModal(null); fetchAppointments(); }} />}
      {modal && modal !== 'create' && <StatusModal appointment={modal} onClose={() => setModal(null)} onSave={() => { setModal(null); fetchAppointments(); }} />}
    </div>
  );
}