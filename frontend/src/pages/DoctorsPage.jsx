...
function DoctorModal({ doctor, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast(err.response?.data?.message || 'Error saving doctor', 'error');
    } finally {
      setLoading(false);
    }
  };
...
export default function DoctorsPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast(err.response?.data?.message || 'Cannot delete doctor', 'error');
    }
  };
    ...
    <div className="page-body">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted">{total} doctor{total !== 1 ? 's' : ''} registered</p>
        </div>
        <button className="btn btn-primary" onClick={() => setModal('create')}>+ Add Doctor</button>
      </div>

      <div className="toolbar">
        <input className="search-input" placeholder="Search by name or specialization..." value={search}
          onChange={e => { setSearch(e.target.value); setPage(1); }} />
        <input className="search-input" placeholder="Filter by specialization..." value={specialization}
          onChange={e => { setSpecialization(e.target.value); setPage(1); }} style={{ maxWidth: 220 }} />
      </div>
                  ...
                  <tr key={d.id}>
                    ...
                    <td>
                      <div className="flex gap-2">
                        ...
                        <button className="btn btn-sm btn-ghost" onClick={() => handleToggle(d.id)}>{d.is_active ? 'Deactivate' : 'Activate'}</button>
      ...
      {modal && (
        <DoctorModal doctor={modal === 'create' ? null : modal} onClose={() => setModal(null)}
